...
function ask_question_and_store_in_env() {
    ...
    echo -e "\033[33m$question\033[0m"
    read answer
    echo "$env_var_name=\"$answer\"" >> "$env_file"
}
...
if [[ ! -f "$env_file" ]]; then
    ...
    function ask_question_and_store_in_env() {
        ...
        echo -e "\033[33m$question\033[0m"
        read answer
        echo "$env_var_name=\"$answer\"" >> "/home/$USER/.env"
    }

    # Poser les questions et stocker les réponses dans le fichier .env
    ask_question_and_store_in_env "Veuillez entrer votre clé API RealDebrid : " RD_API_KEY
    ask_question_and_store_in_env "Veuillez entrer le nom de domaine ou l'adresse IP du serveur Plex : " PLEX_ADDRESS
